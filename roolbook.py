...
def is_move_legal(
    ...
    # Player is only allowed to play the call_sau if it is called by the lead_card or the last card he has
        ...
        not lead
        and game_mode.__class__ == Sauspiel
        and check_player_owns_call_sau(player_cards=player_cards, call_sau=call_sau)
        ...
        if bool_lead_trump:
            ...
            sim_col_avail = similar_color_available(
                lead_card=lead_card, player_cards=player_cards, trumps=trumps
            )
            if sim_col_avail:
                legal_cards = [
                    sim_color
                    for sim_color in player_cards
                ...
                if (
                    ...
                    and check_player_owns_call_sau(
                        player_cards=player_cards, call_sau=call_sau
                    )
                    and lead_card.card_color == call_sau.card_color
                ):
