...
def check_sau_color_available(player_cards: list[Card]) -> bool:
    ...
    for card_color in colors:
        match card_color:
            case Color.EICHEL:
                eichel_count = count_color_cards(
                    player_cards=player_cards,
                    color=card_color,
                    trump_types=[Type.OBER, Type.UNTER],
                )
            case Color.GRUEN:
                gruen_count = count_color_cards(
                    player_cards=player_cards,
                    color=card_color,
                    trump_types=[Type.OBER, Type.UNTER],
                )
            case Color.SCHELLEN:
                schellen_count = count_color_cards(
                    player_cards=player_cards,
                    color=card_color,
                    trump_types=[Type.OBER, Type.UNTER],
                )
...
def check_available_sau_color_decisions(
    ...
    for color in playable_colors.copy():
        ...
        color_count = count_color_cards(
            player_cards=player_cards, color=color, trump_types=[Type.OBER, Type.UNTER]
        )
